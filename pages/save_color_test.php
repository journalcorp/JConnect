...
try {
    ...
    $all_dominant_colors = $inputData['all_dominant_colors'] ?? [$dominant_color];
    ...
    if (is_resource($conn)) {
        ...
        // เพิ่มข้อมูลใหม่สำหรับแต่ละสีที่เป็น dominant
        ...
        foreach ($all_dominant_colors as $color) {
            $insertStmt = sqlsrv_query($conn, $insertSQL, array(
                $employee_id, $evaluation_round, $purple_score, $yellow_score, $blue_score, $green_score,
                $color, $scores_json, $answers
            ));

            if ($insertStmt === false) {
                $sqlsrv_errors = sqlsrv_errors();
                $errors[] = "ไม่สามารถบันทึกข้อมูลสี $color ได้: " . print_r($sqlsrv_errors, true);
            } else {
                $insertCount++;
        ...
        if ($insertCount === 0) {
            echo json_encode(['success' => false, 'message' => 'ไม่สามารถบันทึกข้อมูลได้เลย', 'errors' => $errors]);
        } elseif ($insertCount < count($all_dominant_colors)) {
            $colorList = implode(', ', array_slice($all_dominant_colors, 0, $insertCount));
            echo json_encode([
                'success' => true, 
                'message' => "บันทึกข้อมูลได้บางส่วน (สี: $colorList)",
                'colors_saved' => $insertCount,
                'total_colors' => count($all_dominant_colors),
                'errors' => $errors
            ]);
        } else {
            $colorList = implode(', ', $all_dominant_colors);
            echo json_encode([
                'success' => true, 
                'message' => "บันทึกสำเร็จทั้งหมด (สี: $colorList)",
                'colors_saved' => $insertCount,
                'total_colors' => count($all_dominant_colors)
            ]);
        }
