...
try {
    ...
    foreach ($emp_pe as $emp_pe_item) {
        ...
        if ($evalExists) {
            ...
            $resultId = sqlsrv_query($conn, $sqlMaxId);
            if ($resultId === false) {
                throw new Exception("ไม่สามารถหา ID ถัดไปได้");
            }
            ...
            $result = sqlsrv_query($conn, $sqlInsertEval, array($nextId, $transactionID, $form, $emp_as, $emp_pe_item, $evaluation_period, $start_date, $end_date));
        }
        ...
        // บันทึกข้อมูลลงตาราง form_all_hr
        // แปลง FormID จาก string เป็น number สำหรับตาราง form_all_hr
        $formIdForAllHr = intval(str_replace('F', '', $form)); // F005 -> 5
        
        $sqlCheckFormAllHr = "SELECT COUNT(*) AS count FROM form_all_hr WHERE FormID = ? AND emp_as = ? AND emp_pe = ?";
        $result = sqlsrv_query($conn, $sqlCheckFormAllHr, array($formIdForAllHr, $emp_as, $emp_pe_item));
        ...
        $row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC);
        $formAllHrExists = $row['count'] > 0;
        sqlsrv_free_stmt($result);
        
        // ดึงข้อมูล FormName จากตาราง form
        $sqlGetFormName = "SELECT FormName FROM form WHERE FormID = ?";
        $resultFormName = sqlsrv_query($conn, $sqlGetFormName, array($form));
        ...
        if ($formAllHrExists) {
            ...
            $result = sqlsrv_query($conn, $sqlUpdateFormAllHr, array($formName, $start_date, $end_date, $formIdForAllHr, $emp_as, $emp_pe_item));
            writeLog("Updated form_all_hr for $emp_pe_item");
        } else {
            // หา FormIDmain ถัดไป
            $sqlMaxFormIDmain = "SELECT ISNULL(MAX(FormIDmain), 0) + 1 AS next_id FROM form_all_hr";
            $resultFormIDmain = sqlsrv_query($conn, $sqlMaxFormIDmain);
            if ($resultFormIDmain === false) {
                throw new Exception("ไม่สามารถหา FormIDmain ถัดไปได้");
            }
            $rowFormIDmain = sqlsrv_fetch_array($resultFormIDmain, SQLSRV_FETCH_ASSOC);
            $nextFormIDmain = $rowFormIDmain['next_id'];
            sqlsrv_free_stmt($resultFormIDmain);
            ...
            $result = sqlsrv_query($conn, $sqlInsertFormAllHr, array($nextFormIDmain, $formIdForAllHr, $formName, $emp_as, $emp_pe_item, $start_date, $end_date, $emp_as));
            writeLog("Inserted form_all_hr for $emp_pe_item with FormIDmain $nextFormIDmain");
        }
        
        if ($result === false) {
            $errors = sqlsrv_errors();
            writeLog("form_all_hr operation failed for $emp_pe_item: " . print_r($errors, true));
            throw new Exception("ไม่สามารถบันทึก form_all_hr สำหรับ $emp_pe_item ได้");
        }
        sqlsrv_free_stmt($result);
        writeLog("form_all_hr operation successful for $emp_pe_item");
    }
